...
async function getSeasons() {
  const response = await fetch(`${process.env.REACT_APP_API_SERVER}/getarenaseasons`);
...
class RankGraph extends Component {
  ...
  async getGraph(newSeason) {
    const response = await fetch(`${process.env.REACT_APP_API_SERVER}/getarenagraph?season=${newSeason}`);
    const json = await response.json();
